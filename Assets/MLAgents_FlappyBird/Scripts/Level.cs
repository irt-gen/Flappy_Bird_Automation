...
public class Level : MonoBehaviour
{
    ...
    [Header("Inspector Control")] [SerializeField]
    private float inspectorGapSize = 40f;

    [SerializeField] private float inspectorPipeHeight = 50f;
    [SerializeField] private bool useInspectorValues = true;
    [SerializeField] private bool randomizePipeHeights = false;
    public bool randomizedEnvironment;
    ...
    private void HandlePipeSpawning()
    {
        ...
        if (pipeSpawnTimer < 0)
        {
            ...
            float currentGapSize = randomizedEnvironment ? UnityEngine.Random.Range(40, 81) : inspectorGapSize;
            float height;
    
            
            if (randomizePipeHeights)
            {
                // Randomize height but keep gap size from inspector
                float heightEdgeLimit = 10f;
                float minHeight = inspectorGapSize * .5f + heightEdgeLimit;
                float totalHeight = CAMERA_ORTHO_SIZE * 2f;
                float maxHeight = totalHeight - inspectorGapSize * .5f - heightEdgeLimit;
                
                // Add this check
                if (maxHeight <= minHeight)
                {
                    maxHeight = minHeight + 15f; // Give at least 5 units range
                }
                
                height = UnityEngine.Random.Range(minHeight, maxHeight);
            }
            else if (useInspectorValues)
            {
                height = inspectorPipeHeight; // Direct value from inspector
            }
            else
            {
                // Original random calculation
                float heightEdgeLimit = 10f;
            ...
            CreateGapPipes(height, currentGapSize, PIPE_SPAWN_X_POSITION);
        }
    }

    
    [Header("Auto Gap Adjuster")]
    [SerializeField]private float gapDecreaseTimer = 0f;
    [SerializeField]private float gapDecreaseInterval = 5f;
    [SerializeField]private float gapDecreaseAmount = 5f;
    [SerializeField]private float gapMaxDecrease = 5f;
    
    private void HandleGapSizeDecrease()
    {
        if (state == State.Playing)
        {
            gapDecreaseTimer += Time.fixedDeltaTime;
        
            if (gapDecreaseTimer >= gapDecreaseInterval)
            {
                gapDecreaseTimer = 0f;
                inspectorGapSize = Mathf.Max(gapMaxDecrease, inspectorGapSize - gapDecreaseAmount);
                Debug.Log("Gap Size decreased to: " + inspectorGapSize);
            }
        }
    }
    

    public void SetInspectorValues(float newGapSize, float newPipeHeight)
    {
        inspectorGapSize = newGapSize;
        inspectorPipeHeight = newPipeHeight;
        useInspectorValues = true;
    }
}